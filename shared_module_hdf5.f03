...
function exists(filename,do_not_stop) result(res)
   ...
   logical*4,intent(in),optional :: do_not_stop
   logical*4                     :: res
...
subroutine write_dataset_1d_int4(datasetname,dat,attribute)
   ...
   if (status.ne.0) call error('unable to create HDF5 dataset "'//datasetname//'"')
   call h5dwrite_f(dataset_id,H5T_STD_I32LE,dat,n,status) ! write data set
   if (status.ne.0) call error('unable to write HDF5 dataset "'//datasetname//'"')
...
subroutine write_attribute(attribute,dataset_id)
   ...
   if (status.ne.0) call error('unable to create attribute of HDF5 dataset')
   call h5awrite_f(attr_id,atype_id,attribute,data_dims,status) ! Write the attribute data.
   if (status.ne.0) call error('unable to write attribute of HDF5 dataset')
