...
logical function apparent_selection(sky)
   ...
   select case (trim(para%name))
   case ('DEVILS')
      apparent_selection = sky%galaxy%mag<=21.2
   case default
...
function convert_properties(base,sam,id) result(sky)
   ...
   ! sky coordinates
   sky%galaxy%dc  = base%dc         ! [Mpc/h]
   sky%galaxy%ra  = base%ra         ! [rad]
   sky%galaxy%dec = base%dec        ! [rad]
   ...
   sky%galaxy%id_halo_sky     = sam%galaxy%id_halo+base%tile*int(1e10,8)
   sky%galaxy%id_halo_sam     = sam%galaxy%id_halo
   sky%galaxy%tile             = base%tile
   sky%galaxy%snapshot        = base%snapshot
   sky%galaxy%subsnapshot     = base%subsnapshot
